...
void annealing2(int iresult, int arch, bool* borders){
    int *localGrid, *localPositions;
    int currentCost = cost, nextCost, swapCount=0, increment, distManhattanI, distManhattanJ;
...
int main(int argc, char** argv) {
    ...
    //Precalculations
    dim = ceil(sqrt(nodes))+2;
    buildMatrices(dim);
    gridSize = dim*dim;

    //Results for mesh and hop architectures
    ...
    for (int k = 0; k < num_arch; ++k) {
        ...
        for(int i=0; i<NGRIDS; i++){
            ...
            for(int j=0; j<nodes; j++){
                ...
                for(int t=0; t<gridSize; t++){
                    if(borders[t]==true && setNodes[j]==false && grid[i][t]==EMPTY){
                        grid[i][t]=j;
                        setNodes[j] = true;
                        break;
                    }
            ...
            for(int j=0; j<gridSize; j++){
                for(int t=0; t<gridSize; t++){
                    ...
                    if(aleat > 0.2 && (borders[j]==true && borders[t]==true)){
                        int temp = grid[i][j];
                        grid[i][j] = grid[i][t];
                        grid[i][t] = temp;
                    }
        ...
        if(k==0){
            ...
            ofile << "#----------\t" << "--\t" << "--\t" << "------\t" << "------------\n";
            for(int i=0; i<nodes; i++){
                ofile << names[i] << "\t\t" << positions[idxMinCost][i]%dim << "\t" << positions[idxMinCost][i]/dim << "\n";
            }             
        } else{
            ofile1.open(buffer1);
            ...
            ofile1 << "#----------\t" << "--\t" << "--\t" << "------\t" << "------------\n";
            for(int i=0; i<nodes; i++){
                ofile1 << names[i] << "\t\t" << positions[idxMinCost][i]%dim << "\t" << positions[idxMinCost][i]/dim << "\n";
            } 
        }
        ofile.close();
        ofile1.close();
    ...
    printf("%s,%d,%d,%.2lf,%.2lf\n", argv[3], cost_min[0], cost_min[1], time_total[0], time_total[1]);
    return 0;
}